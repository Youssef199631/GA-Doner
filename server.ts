...
async function startServer() {
  ...
  app.post('/api/send-order-email', async (req, res) => {
    ...
    const SMTP_HOST = 'mail.immo-khattabi-conseil.ma';
    const SMTP_PORT = 465;
    const SMTP_USER = 'contact@immo-khattabi-conseil.ma';
    const SMTP_PASS = 'Immo.Maroc';
    ...
    if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
      try {
        ...
        emailSent = true;
        console.log('[EMAIL] Successfully sent notifications');
      } catch (err: any) {
        console.error('[EMAIL] Error:', err.message);
        errors.push(`Email error: ${err.message}`);
      }
    } else {
      console.warn('[EMAIL] SMTP not fully configured in Secrets.');
    }
    ...
    if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_FROM_NUMBER) {
      try {
        ...
        console.log(`[SMS] Sent to ${ownerPhoneNumber}`);
        smsSent = true;
      } catch (err: any) {
      ...
      console.warn('[SMS] Twilio not fully configured in Secrets.');
    }
